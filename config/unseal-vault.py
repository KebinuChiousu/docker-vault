...
configFile = os.path.join("/vault/config/", "vault-config.json")
...
def vaultInitialize(shares=5, threshold=3):
...
def saveVaultConfiguration(content):  
  ...
  file.write(content)
  file.close()
  print("Config written to: {0}".format(configFile))

def checkHttpStatus(req, status=requests.codes.ok): # pylint: disable=E1101
